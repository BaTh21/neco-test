...
class _MediaGalleryState extends State<MediaGallery> {
  List<MediaItem> get _mediaItems {
    ...
    for (int i = 0; i < widget.videos.length; i++) {
      items.add(MediaItem(
        url: widget.videos[i],
        isVideo: true,
        thumbnailUrl: widget.videoThumbnails != null && i < widget.videoThumbnails!.length
        ...
        child: Row(
          children: [
            ...
            Expanded(
              child: Column(
                children: [
                  Expanded(child: _buildItem(items[1], 1, isFirst: true)),
                  const SizedBox(height: 2),
                  Expanded(child: _buildItem(items[2], 2, isLast: true)),
                ],
              ),
            ),
          ],
      ...
      child: Row(
        children: [
          Expanded(
            child: Column(
              children: [
                Expanded(child: _buildItem(items[0], 0, isFirst: true)),
                const SizedBox(height: 2),
                Expanded(child: _buildItem(items[1], 1)),
              ],
            ),
          ),
          const SizedBox(width: 2),
          Expanded(
            child: Column(
              children: [
                Expanded(child: _buildItem(items[2], 2)),
                const SizedBox(height: 2),
                Expanded(
                  child: Stack(
                    children: [
  ...
  // FIX: Improved video thumbnail with better fallback
...
class MediaItem {
  ...
  MediaItem({required this.url, required this.isVideo, this.thumbnailUrl});
}