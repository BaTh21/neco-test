...
def upload_video_to_cloudinary(video_data: bytes, folder: str = "videos") -> Dict[str, Any]:
    """Upload video to Cloudinary and return (video_url, thumbnail_url) - thumbnail always a string."""
    try:
        base_folder = os.getenv('CLOUDINARY_UPLOAD_FOLDER', 'whisper_space')
        ...
        try:
            ...
            if not thumbnail_url:
                thumbnail_url = "https://res.cloudinary.com/your-cloud-name/image/upload/v1/default-video-thumb.jpg"
                print("Using default thumbnail placeholder")
...
def generate_video_thumbnail(video_url: str) -> str:
    """Generate a guaranteed image URL for a video thumbnail."""
    try:
        public_id = extract_public_id_from_url(video_url)
        if not public_id:
        ...
        print(f"Failed to generate thumbnail: {str(e)}")
        return "https://res.cloudinary.com/your-cloud-name/image/upload/v1/default-video-thumb.jpg"
